<script>
	import { calmethod } from '../Trapezoidalcom/cal.js';

	let a = 0;
	let b = 10;
	let n = 0;
	let func = '4*x^5 - 3*x^4 + x^3 - 6*x + 2';
	let result = {
		i: 0,
		truevalue: 0,
		calerror: 0
	...
	function calculate() {
		result = calmethod(a, b, n, func);
	}
</script>

<h1 class="text-4xl text-primary font-bold flex justify-center pt-5">Composite Trapezoidal Rule</h1>
...
<div class="flex justify-center my-10">
	<div class="lg:flex lg:space-x-4 md:flex md:space-x-4">
		...
		<form on:submit|preventDefault={calculate} class="lg:flex lg:space-x-4 md:flex md:space-x-4">
			...
			<div>
				<label class="block">N value</label>
				<input
					type="number"
					bind:value={n}
					placeholder="10"
